<template>
  <div class="bg-pink rounded-3xl py-4 px-6 md:px-10 md:py-5 flex flex-col md:flex-row justify-between max-md:gap-4">
    <div v-if="!requestPending" >
      ...
      <p class="uppercase text-white tracking-widest md:text-lg">{{ guest.rsvp === 'yes' ? "Accepted" : "Declined"}}</p>
      <p class="uppercase text-white tracking-widest md:text-lg">{{ guest.email }}</p>
      <p v-if="guest.rsvp === 'yes'" class="uppercase text-white tracking-widest md:text-lg">{{ guest.dietary_requirements ? guest.dietary_requirements : 'No Dietary Requirements' }}</p>
    </div>
